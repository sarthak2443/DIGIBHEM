...
function eraserMouseMove(e) {
  if (toolState == 2) {
    if (eraserMouseDownState) {
      let x = e.clientX;
      let y = e.clientY;
      let eraserSize = 20; // Adjust the size of the eraser brush as desired
      // Erase Content - Code
      tool.globalCompositeOperation = 'destination-out';
      tool.beginPath();
      tool.arc(x, y, eraserSize, 0, 2 * Math.PI); // Increase the size of the arc to increase the area of erasing
      tool.fill();
      // Additional logic to erase in a line
      tool.beginPath();
